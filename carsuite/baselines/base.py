...
class SetPointAgent(carsuite.Agent):
  """An agent that predicts setpoints and consumes them with a PID
  controller."""

  def __init__(
      self,
      environment: carsuite.envs.CARLAEnv,
      *,
    ...
    self._vehicle_controller = carsuite.VehiclePIDController(
  ...
  @abc.abstractmethod
  def __call__(self, observation: carsuite.Observations, *args,
               **kwargs) -> np.ndarray:
    """Returns the predicted plan in ego-coordinates, based on a model."""

  def act(self, observation: carsuite.Observations, *args,
          **kwargs) -> carsuite.Action:
