...
class AutopilotAgent(carsuite.Agent):
  ...
  def __init__(self,
    ...
    self._local_planner = carsuite.LocalPlanner(
        self._vehicle,
        opt_dict=dict(
  ...
  def act(
      self,
      observation: carsuite.Observations,
  ) -> carsuite.Action:
  ...
  def _is_vehicle_hazard(
    ...
    for target_vehicle in vehicle_list:
      ...
      if carsuite.is_within_distance_ahead(
          loc,
  ...
  def _is_light_red_us_style(self, lights_list, debug=False):
    ...
    if self._local_planner.target_waypoint is not None:
      if self._local_planner.target_waypoint.is_junction:
        ...
        for traffic_light in lights_list:
          loc = traffic_light.get_location()
          magnitude, angle = carsuite.compute_magnitude_angle(
