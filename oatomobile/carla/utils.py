...
def setup(
...
) -> Tuple[carla.Client, subprocess.Popen]:  # pylint: disable=no-member
  ...
  Returns:
    ...
    server_process: The `CARLA` server process pointer.
  """
  if town is not None:
  ...
  while attempts < num_max_restarts:
    ...
    atexit.register(os.killpg, server_process.pid, signal.SIGKILL)
    ...
    try:
      ...
      atexit.unregister(lambda: os.killpg(server_process.pid, signal.SIGKILL))
...
def global_plan(
  ...
  try:
    from agents.navigation.global_route_planner import \
        GlobalRoutePlanner  # pylint: disable=import-error
    from agents.navigation.global_route_planner_dao import \
        GlobalRoutePlannerDAO  # pylint: disable=import-error
  except ImportError:
    raise ImportError(
