...
class CARLADataset(core.Dataset):
  ...
  def process(
    ...
    for episode_token in tqdm.tqdm(os.listdir(dataset_dir)):
      ...
      episode = core.Episode(parent_dir=dataset_dir, token=episode_token)
      ...
      for i in tqdm.trange(
        ...
        try:
          ...
          player_past = cutils.world2local(
          ...
          player_future = cutils.world2local(
              current_location=current_location,
              current_rotation=current_rotation,
  ...
  def plot_datum(
    ...
    Args:
      ...
      output_dir: The full path to the output directory.
    """
    from oatomobile.carla import renderer as gutils
    ...
    if "lidar" in datum:
      bev_meters = 25.0
      lidar = gutils.lidar_2darray_to_rgb(datum["lidar"])
